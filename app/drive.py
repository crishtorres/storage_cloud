...
def getFullPath(path):
    return UPLOAD_FOLDER[0:-1] + path if UPLOAD_FOLDER[-1:] == '\'' or UPLOAD_FOLDER[-1:] == '/' else UPLOAD_FOLDER + path
...
def getRoot(user, path):
    ...
    if path == UPLOAD_FOLDER or path == UPLOAD_FOLDER+"/":
        ...
        prev_path = '' if tmp_path == UPLOAD_FOLDER else '..'
...
def upload_file(user):
    if request.method == 'POST':
        ...
        dest = UPLOAD_FOLDER if dest == '' else getFullPath(dest)
        ...
        try:
            ...
            res = {'msg': 'Error al subir el archivo : ' +
                   error.strerror, 'status': 404}
    ...
    if request.method == 'POST':
        ...
        if os.path.isdir(path):
            ...
            try:
                ...
                print(error)
                res = {'msg': 'Error al crear directorio : ' +
                       error.strerror, 'status': 404}
        ...
        if type_in == 'file':
            if not os.path.isfile(path):
                ...
                try:
                    ...
                    print(error)
                    res = {'msg': 'Error al eliminar el archivo : ' +
                           error.strerror, 'status': 404}
        else:
            if not os.path.isdir(path):
                ...
                try:
                    os.rmdir(path)
                    res = {'msg': 'Directorio eliminado correctamente!',
                           'status': 200}
                except OSError as error:
                    print(error)
                    res = {'msg': 'Error al eliminar directorio : ' +
                           error.strerror, 'status': 404}
...
def download_file(user, filename):
    ...
    download = getFullPath(os.path.split(filename)[0])
